...
export default function Verify() {
  ...
  const markAttendance = async (name: string) => {
    try {
      ...
      // Wait for 2-3 seconds and show success toast
      setTimeout(() => {
        Alert.alert(
          "Attendance Recorded",
          `${predictName}, Your attendance is recorded for today. Tap OK to confirm and return home.`,
          [
            {
              text: "OK",
              onPress: () => {
  ...
  const attendance = async (base64: string) => {
    try {
      const res = await axios.post(`${env.REACT_NATIVE_API_URL}/attendance`, {
        photo: base64,
      });
        ...
        <CameraView ref={cameraRef} style={styles.camera} facing="front">
          ...
          {isStart ? (
            <View style={styles.progressBarWrapper}>
              <Text style={styles.predictName}>
                Bạn là:{" "}
                <Text
                  style={{ fontSize: 18, color: "#EF6F8B", fontWeight: "600" }}
                >
                  {predictName}
                </Text>
              </Text>
